...
public class UserServiceImp implements IUserService {
   ...
   public ServerResponse selectQuestion(String username){
    ...
    public ServerResponse<String> resetPassword(String password,String passwordNew,User user){
        //为了防止发生横向越权，需要校验一下这个用户的旧密码，一定要指定这个用户，因为我们会查询一个Count（1），如果不指定ID，那么结果就是true。因为COUNT>0
    ...
    public ServerResponse<User> updateInformation(User user){
        //username是不能被更新的
        //eamil也要进行校验，校验新的email是不是已经存在，并且存在的eamil如果相同的话，不能是我们当前的用户
        int resultCount = userMapper.checkEmailByUserId(user.getEmail(),user.getId());
        if (resultCount>0){
            return ServerResponse.createByErrorMessage("Email已经存在，请更换Email再重新尝试");
        }
